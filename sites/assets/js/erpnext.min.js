...
$.extend(erpnext, {
	...
	stale_rate_allowed: function stale_rate_allowed() {
...
$.extend(erpnext.utils, {
	...
	make_subscription: function make_subscription(doctype, docname) {
		frappe.call({
	...
	first_row_is_empty: function first_row_is_empty(child_table) {
	...
	remove_empty_first_row: function remove_empty_first_row(frm, child_table_name) {
...
erpnext.taxes_and_totals = erpnext.payments.extend({
		...
		$.each(this.frm.doc["items"] || [], function (n, item) {
			...
			$.each(me.frm.doc["taxes"] || [], function (i, tax) {
				...
				if (n == me.frm.doc["items"].length - 1) {
					...
					me.set_in_company_currency(tax, ["total", "tax_amount", "tax_amount_after_discount_amount"]);
	...
	set_cumulative_total: function set_cumulative_total(row_idx, tax) {
		...
		if (this.frm.doc["taxes"] && this.frm.doc["taxes"].length) {
			...
			if (any_inclusive_tax) {
				...
				if (diff && Math.abs(diff) <= 5.0 / Math.pow(10, precision("tax_amount", last_tax))) {
					this.frm.doc.rounding_adjustment = flt(flt(this.frm.doc.rounding_adjustment) + diff, precision("rounding_adjustment"));
				}
			}
		}
	},
	...
	set_rounded_total: function set_rounded_total() {
		...
		if (this.frm.doc.discount_amount) {
			...
			if (total_for_discount_amount) {
				$.each(this.frm.doc["items"] || [], function (i, item) {
					...
					if ((!(me.frm.doc.taxes || []).length || me.frm.doc.apply_discount_on == "Net Total") && i == (me.frm.doc.items || []).length - 1) {
	...
	calculate_change_amount: function calculate_change_amount() {
		...
		if (this.frm.doc.doctype == "Sales Invoice" && this.frm.doc.paid_amount > this.frm.doc.grand_total && !this.frm.doc.is_return) {
...
erpnext.TransactionController = erpnext.taxes_and_totals.extend({
	setup: function setup() {
		...
		if (this.frm.fields_dict["payment_terms_template"] && this.frm.fields_dict["payment_schedule"] && this.frm.doc.payment_terms_template && !this.frm.doc.payment_schedule.length) {
	...
	onload: function onload() {
		...
		if (this.frm.doc.__islocal) {
			...
			var set_value = function set_value(fieldname, value) {
			...
			return frappe.run_serially([function () {
				return set_value('currency', currency);
			}, function () {
				return set_value('price_list_currency', currency);
			}, function () {
				return set_value('status', 'Draft');
			}, function () {
				return set_value('is_subcontracted', 'No');
			}, function () {
				if (_this.frm.doc.company && !_this.frm.doc.amended_from) {
					_this.frm.trigger("company");
				}
			}]);
		}
	},
				...
				callback: function callback(r) {
					if (!r.exc && r.message) {
						frappe.run_serially([function () {
							...
							return me.calculate_taxes_and_totals();
						}]);
					}
				}
			});
					...
					callback: function callback(r) {
						if (!r.exc) {
							frappe.run_serially([function () {
								...
								return me.toggle_conversion_factor(item);
							}, function () {
								...
								return me.conversion_factor(doc, cdt, cdn, true);
							}]);
						}
					}
				});
	...
	company: function company() {
		...
		var set_pricing = function set_pricing() {
			if (me.frm.doc.company && me.frm.fields_dict.currency) {
				...
				frappe.run_serially([function () {
					return me.frm.script_manager.trigger("currency");
				}, function () {
					return me.apply_default_taxes();
				}, function () {
					return me.apply_pricing_rule();
				}]);
			}
		};
	...
	recalculate_terms: function recalculate_terms() {
	...
	shipping_rule: function shipping_rule() {
		var _this3 = this;
		...
		if (this.frm.doc.shipping_rule) {
			return this.frm.call({
				...
				return _this3.frm.set_value('shipping_rule', '');
			});
		} else {
			me.calculate_taxes_and_totals();
		}
	...
	set_actual_charges_based_on_currency: function set_actual_charges_based_on_currency() {
		var me = this;
		$.each(this.frm.doc.taxes || [], function (i, d) {
			if (d.charge_type == "Actual") {
				frappe.model.set_value(d.doctype, d.name, "tax_amount", flt(d.tax_amount) / flt(me.frm.doc.conversion_rate));
			}
		});
	},
	...
	calculate_net_weight: function calculate_net_weight() {
		var me = this;
	...
	apply_price_list: function apply_price_list(item) {
		...
		return this.frm.call({
			...
			callback: function callback(r) {
				if (!r.exc) {
					frappe.run_serially([function () {
						return me.frm.set_value("price_list_currency", r.message.parent.price_list_currency);
					}, function () {
						return me.frm.set_value("plc_conversion_rate", r.message.parent.plc_conversion_rate);
					}, function () {
	...
	payment_terms_template: function payment_terms_template() {
		var me = this;
		if (this.frm.doc.payment_terms_template) {
			var posting_date = this.frm.doc.bill_date || this.frm.doc.posting_date || this.frm.doc.transaction_date;
	...
	payment_term: function payment_term(doc, cdt, cdn) {
		var row = locals[cdt][cdn];
		if (row.payment_term) {
			frappe.call({
...
erpnext.ItemSelector = Class.extend({
	...
	add_item: function add_item(item_code) {
		...
		$.each(this.frm.doc.items || [], function (i, d) {
			if (d[_this.item_field] === item_code) {
		...
		if (!added) {
			var d = null;
			frappe.run_serially([function () {
				d = _this.grid.add_new_row();
			}, function () {
				return frappe.model.set_value(d.doctype, d.name, _this.item_field, item_code);
			}, function () {
...
frappe.help.help_links['Form/BOM Update Tool'] = [{ label: 'BOM Update Tool', url: 'https://frappe.github.io/erpnext/user/manual/en/manufacturing/tools/bom-update-tool' }];
...
agriculture.TernaryPlot = function () {
	function TernaryPlot(opts) {
		var _this = this;

		_classCallCheck(this, TernaryPlot);
		...
		frappe.require('assets/frappe/js/lib/snap.svg-min.js', function () {
			...
			_this.init_config();
			_this.make_plot();
			_this.make_plot_marking();
			_this.make_legend();
			_this.mark_blip();
		});
	}

	_createClass(TernaryPlot, [{
		key: 'make_svg',
		value: function make_svg() {
			this.$svg = $('<svg height="350" width="400">');
			$(this.parent).append(this.$svg);
		}
	}, {
		key: 'init_snap',
		value: function init_snap() {
			this.paper = new Snap(this.$svg.get(0));
		}
	}, {
		key: 'init_config',
		value: function init_config() {
			this.config = {
				triangle_side: 300,
				spacing: 50,
				strokeWidth: 1,
			...
			var _config = this.config,
			    t = _config.triangle_side,
			    s = _config.spacing,
			    p = _config.scaling_factor;
			...
			this.coords = {
				...
				loamy_sand: {
					points: [s + 15 * p * Snap.cos(60), s + (100 - 15) * p * Snap.cos(30), s + 10 * p * Snap.cos(60), s + (100 - 10) * p * Snap.cos(30), s + (100 - 85) * p, s + t * Snap.cos(30), s + (100 - 70) * p, s + t * Snap.cos(30)],
					color: frappe.ui.color.get('pink')
				},
				sandy_loam: {
					points: [s + 20 * p * Snap.cos(60) + 27.5 * p, s + (100 - 20) * p * Snap.cos(30), s + 20 * p * Snap.cos(60), s + (100 - 20) * p * Snap.cos(30), s + 15 * p * Snap.cos(60), s + (100 - 15) * p * Snap.cos(30), s + (100 - 75) * p, s + t * Snap.cos(30), s + (100 - 50) * p, s + t * Snap.cos(30), s + (100 - 50) * p + 7.5 * p * Snap.cos(60), s + t * Snap.cos(30) - 7.5 * p * Snap.cos(30), s + (100 - 50) * p + 7.5 * p * Snap.cos(60) - 10 * p, s + t * Snap.cos(30) - 7.5 * p * Snap.cos(30)],
					color: frappe.ui.color.get('pink', 'light')
				},
				loam: {
					points: [s + (100 - 50) * p + 27.5 * p * Snap.cos(60), s + t * Snap.cos(30) - 27.5 * p * Snap.cos(30), s + (100 - 50) * p + 27.5 * p * Snap.cos(60) - 22.5 * p, s + t * Snap.cos(30) - 27.5 * p * Snap.cos(30), s + 20 * p * Snap.cos(60) + 27.5 * p, s + (100 - 20) * p * Snap.cos(30), s + (100 - 50) * p + 7.5 * p * Snap.cos(60) - 10 * p, s + t * Snap.cos(30) - 7.5 * p * Snap.cos(30), s + (100 - 50) * p + 7.5 * p * Snap.cos(60), s + t * Snap.cos(30) - 7.5 * p * Snap.cos(30)],
					color: frappe.ui.color.get('brown')
				},
				silt_loam: {
					points: [s + t - 27.5 * p * Snap.cos(60), s + 72.5 * p * Snap.cos(30), s + (100 - 50) * p + 27.5 * p * Snap.cos(60), s + t * Snap.cos(30) - 27.5 * p * Snap.cos(30), s + (100 - 50) * p, s + t * Snap.cos(30), s + (100 - 20) * p, s + t * Snap.cos(30), s + (100 - 20) * p + 12.5 * p * Snap.cos(60), s + 90 * p * Snap.cos(30), s + t - 12.5 * p * Snap.cos(60), s + (100 - 12.5) * p * Snap.cos(30)],
					color: frappe.ui.color.get('green', 'dark')
				},
				silt: {
					points: [s + t - 12.5 * p * Snap.cos(60), s + (100 - 12.5) * p * Snap.cos(30), s + (100 - 20) * p + 12.5 * p * Snap.cos(60), s + 90 * p * Snap.cos(30), s + (100 - 20) * p, s + t * Snap.cos(30), s + t, s + t * Snap.cos(30)],
					color: frappe.ui.color.get('green')
				},
				silty_clay_loam: {
					points: [s + t - 40 * p * Snap.cos(60), s + 60 * p * Snap.cos(30), s + t - 40 * p * Snap.cos(60) - 20 * p, s + 60 * p * Snap.cos(30), s + t - 27.5 * p * Snap.cos(60) - 20 * p, s + 72.5 * p * Snap.cos(30), s + t - 27.5 * p * Snap.cos(60), s + 72.5 * p * Snap.cos(30)],
					color: frappe.ui.color.get('cyan', 'dark')
				},
				silty_clay: {
					points: [s + t - 60 * p * Snap.cos(60), s + 40 * p * Snap.cos(30), s + t - 40 * p * Snap.cos(60) - 20 * p, s + 60 * p * Snap.cos(30), s + t - 40 * p * Snap.cos(60), s + 60 * p * Snap.cos(30)],
					color: frappe.ui.color.get('cyan')
				},
				clay_loam: {
					points: [s + t - 40 * p * Snap.cos(60) - 20 * p, s + 60 * p * Snap.cos(30), s + t - 40 * p * Snap.cos(60) - 45 * p, s + 60 * p * Snap.cos(30), s + t - 27.5 * p * Snap.cos(60) - 45 * p, s + 72.5 * p * Snap.cos(30), s + t - 27.5 * p * Snap.cos(60) - 20 * p, s + 72.5 * p * Snap.cos(30)],
					color: frappe.ui.color.get('green', 'light')
				},
				sandy_clay_loam: {
					points: [s + 35 * p * Snap.cos(60) + 20 * p, s + (100 - 35) * p * Snap.cos(30), s + 35 * p * Snap.cos(60), s + (100 - 35) * p * Snap.cos(30), s + 20 * p * Snap.cos(60), s + (100 - 20) * p * Snap.cos(30), s + 20 * p * Snap.cos(60) + 27.5 * p, s + (100 - 20) * p * Snap.cos(30), s + t - 27.5 * p * Snap.cos(60) - 45 * p, s + 72.5 * p * Snap.cos(30)],
					color: frappe.ui.color.get('pink', 'dark')
				},
				sandy_clay: {
					points: [s + 55 * p * Snap.cos(60), s + (100 - 55) * p * Snap.cos(30), s + 35 * p * Snap.cos(60), s + (100 - 35) * p * Snap.cos(30), s + 35 * p * Snap.cos(60) + 20 * p, s + (100 - 35) * p * Snap.cos(30)],
					color: frappe.ui.color.get('red')
				},
				clay: {
					points: [s + t * Snap.cos(60), s, s + 55 * p * Snap.cos(60), s + (100 - 55) * p * Snap.cos(30), s + t - 40 * p * Snap.cos(60) - 45 * p, s + 60 * p * Snap.cos(30), s + t - 40 * p * Snap.cos(60) - 20 * p, s + 60 * p * Snap.cos(30), s + t - 60 * p * Snap.cos(60), s + 40 * p * Snap.cos(30)],
		...
		key: 'get_coords',
		value: function get_coords(soil_type) {
			return this.coords[soil_type].points;
		}
	}, {
		key: 'get_color',
		value: function get_color(soil_type) {
			return this.coords[soil_type].color;
		}
	}, {
		key: 'make_plot',
		value: function make_plot() {
		...
		key: 'make_plot_marking',
		value: function make_plot_marking() {
			var _config2 = this.config,
			    t = _config2.triangle_side,
			    s = _config2.spacing,
			    p = _config2.scaling_factor;


			var clay = this.paper.text(t * Snap.cos(60) / 2, s + t * Snap.cos(30) / 2, "Clay").attr({
				fill: frappe.ui.color.get('black')
			});
			clay.transform("r300");

			var silt = this.paper.text(t, s + t * Snap.cos(30) / 2, "Silt").attr({
				fill: frappe.ui.color.get('black')
			});
			silt.transform("r60");

			var sand = this.paper.text(35 + t * Snap.cos(60), 90 + t * Snap.cos(30), "Sand").attr({
				fill: frappe.ui.color.get('black')
			});
			sand.transform("r0");
		}
		...
		value: function mark_blip() {
			...
			if (clay + sand + silt != 0) {
				var _config3 = this.config,
				    t = _config3.triangle_side,
				    s = _config3.spacing,
				    p = _config3.scaling_factor;
...
frappe.ui.form.ItemQuickEntryForm = frappe.ui.form.QuickEntryForm.extend({
	...
	init_post_render_dialog_operations: function init_post_render_dialog_operations() {
	...
	insert: function insert(variant_values) {
		var me = this;
	...
	get_variant_fields: function get_variant_fields() {
		var variant_fields = [{
	...
	get_manufacturing_fields: function get_manufacturing_fields() {
	...
	get_attributes_fields: function get_attributes_fields() {
	...
	init_for_create_variant_trigger: function init_for_create_variant_trigger() {
		var me = this;
	...
	preset_fields_for_template: function preset_fields_for_template() {
		var _this = this;
		...
		['item_code', 'item_name', 'item_group', 'stock_uom'].forEach(function (d) {
			var f = _this.dialog.get_field(d);
	...
	init_for_item_template_trigger: function init_for_item_template_trigger() {
		var me = this;
	...
	init_post_template_trigger_operations: function init_post_template_trigger_operations(is_manufacturer, attributes, attributes_flag) {
	...
	toggle_manufacturer_fields: function toggle_manufacturer_fields() {
		var me = this;
	...
	initiate_render_attributes: function initiate_render_attributes() {
		this.dialog.fields_dict.attribute_html.$wrapper.find(".attributes").empty();
		this.render_attributes(this.attributes);
	},

	render_attributes: function render_attributes(attributes) {
		var me = this;
	...
	init_make_control: function init_make_control(fieldtype, row) {
	...
	init_awesomplete_for_attribute: function init_awesomplete_for_attribute(row) {
		var me = this;
		...
		this[row.attribute].$input.on('input', function (e) {
			frappe.call({
				...
				args: {
					...
					filters: [["parent", "=", $(e.target).attr("data-fieldname")], ["attribute_value", "like", e.target.value + "%"]],
	...
	get_variant_doc: function get_variant_doc() {
		var me = this;
		var variant_doc = {};
	...
	validate_mandatory_attributes: function validate_mandatory_attributes() {
		var me = this;
		var attribute = {};
		var mandatory = [];
